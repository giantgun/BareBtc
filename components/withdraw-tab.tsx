...
export function WithdrawTab({ depositAmount, depositPoolBalance , onWithdraw }: { depositAmount: number; depositPoolBalance: number; onWithdraw?: () => void }) {
  const { toast } = useToast()
  const {poolContractAdrress, sbtcTokenContractAddress, poolContractName, reload} = useWallet()
      ...
      <CardContent className="space-y-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-muted-foreground">Max: {`${currentAllocation().toPrecision(3)}`}sBTC</div>
          </div>
        ...
        <div className="rounded-lg border p-4 space-y-2">
          <div className="flex justify-between">
            ...
            <span className="font-medium">{(currentAllocation() - amount).toPrecision(3)} sBTC</span>
          </div>
        </div>
      </CardContent>
      <CardFooter>
